...
const ALRLabelCreator: QuartzComponent = (_props: QuartzComponentProps) => {
  const css = `
    #alr-label-tool { font-family: var(--codeFont), 'Courier New', monospace; }
    .lc-wrap { border: 1px solid var(--lightgray); border-radius: 6px; overflow: hidden; margin: 1.5rem 0; }
    .lc-header { background: var(--lightgray); border-bottom: 1px solid var(--lightgray); padding: 10px 16px; display: flex; align-items: center; gap: 10px; }
    .lc-header-title { font-size: 11px; font-weight: 700; color: #cc785c; letter-spacing: 2.5px; text-transform: uppercase; }
    .lc-header-sub { font-size: 10px; color: var(--gray); letter-spacing: 1px; margin-left: auto; }
    .lc-body { display: grid; grid-template-columns: 280px 1fr; min-height: 500px; }
    @media (max-width: 700px) { .lc-body { grid-template-columns: 1fr; } .lc-panel-left { border-right: none; border-bottom: 1px solid var(--lightgray); } }
    .lc-panel-left { border-right: 1px solid var(--lightgray); padding: 14px; display: flex; flex-direction: column; gap: 10px; overflow-y: auto; max-height: 560px; }
    .lc-field-group { display: flex; flex-direction: column; gap: 4px; }
    .lc-field-label { font-size: 9px; color: var(--gray); letter-spacing: 1.5px; text-transform: uppercase; }
    .lc-req { color: #cc785c; margin-left: 4px; }
    .lc-input { background: var(--light); border: 1px solid var(--lightgray); color: var(--dark); font-family: var(--codeFont), 'Courier New', monospace; font-size: 11px; padding: 6px 8px; border-radius: 3px; width: 100%; outline: none; transition: border-color 0.15s; box-sizing: border-box; }
    .lc-input:focus { border-color: #cc785c; }
    .lc-section-rule { height: 1px; background: var(--lightgray); margin: 4px 0; }
    .lc-section-label { font-size: 9px; color: #cc785c; letter-spacing: 2px; text-transform: uppercase; margin-top: 2px; }
    .lc-color-row { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }
    .lc-color-field { display: flex; flex-direction: column; gap: 3px; }
    .lc-color-wrap { display: flex; align-items: center; gap: 6px; background: var(--light); border: 1px solid var(--lightgray); border-radius: 3px; padding: 3px 6px; }
    .lc-color-wrap input[type="color"] { width: 20px; height: 20px; border: none; background: none; cursor: pointer; padding: 0; border-radius: 2px; }
    .lc-color-hex { font-size: 10px; color: var(--dark); font-family: var(--codeFont), 'Courier New', monospace; background: none; border: none; outline: none; width: 62px; }
    .lc-panel-right { display: flex; flex-direction: column; }
    .lc-tab-row { display: flex; border-bottom: 1px solid var(--lightgray); }
    .lc-tab { font-size: 9px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; padding: 8px 14px; cursor: pointer; color: var(--gray); border-bottom: 2px solid transparent; transition: all 0.15s; background: none; border-top: none; border-left: none; border-right: none; font-family: var(--codeFont), 'Courier New', monospace; }
    .lc-tab.active { color: #cc785c; border-bottom-color: #cc785c; }
    .lc-tab:hover:not(.active) { color: var(--dark); }
    .lc-preview-area { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 16px; padding: 20px; background: var(--lightgray); min-height: 200px; }
    .lc-preview-label { font-size: 9px; color: var(--gray); letter-spacing: 2px; text-transform: uppercase; align-self: flex-start; }
    .lc-svg-wrap { width: 100%; max-width: 520px; border: 1px solid var(--lightgray); border-radius: 4px; overflow: hidden; background: #fff; }
    .lc-svg-wrap svg { width: 100%; height: auto; display: block; }
    .lc-divider { width: 100%; max-width: 520px; display: flex; align-items: center; gap: 8px; }
    .lc-divider-line { flex: 1; border-top: 1px dashed var(--gray); }
    .lc-divider-label { font-size: 9px; color: var(--gray); letter-spacing: 1.5px; }
    .lc-action-bar { border-top: 1px solid var(--lightgray); padding: 10px 16px; display: flex; gap: 8px; align-items: center; background: var(--lightgray); flex-wrap: wrap; }
    .lc-btn { font-family: var(--codeFont), 'Courier New', monospace; font-size: 10px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; padding: 7px 12px; border-radius: 3px; cursor: pointer; border: 1px solid; transition: all 0.15s; }
    .lc-btn-primary { background: #cc785c; border-color: #cc785c; color: #fff; }
    .lc-btn-primary:hover { background: #b86848; border-color: #b86848; }
    .lc-btn-ghost { background: transparent; border-color: var(--lightgray); color: var(--gray); }
    ...
    .lc-preset-btn { font-family: var(--codeFont), 'Courier New', monospace; font-size: 10px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; padding: 8px 0; border-radius: 3px; cursor: pointer; border: 1px solid var(--lightgray); background: transparent; color: var(--gray); transition: all 0.15s; flex: 1; }
...
(function() {
  ...
  window.lcApplyPreset = function(name) {
    var p = PRESETS[name];
    lcSet('lc-cbody','lc-hbody',p.body); lcSet('lc-czone','lc-hzone',p.zone);
    lcSet('lc-caccent','lc-haccent',p.accent); lcSet('lc-cgrade','lc-hgrade',p.grade);
    lcSet('lc-ctext','lc-htext',p.text); lcSet('lc-cmuted','lc-hmuted',p.muted);
  ...
  function lcSet(cid,hid,val) { document.getElementById(cid).value=val; document.getElementById(hid).value=val; }
  ...
  function getV() {
    return { name:g('lc-name').value||'CARD NAME', set:g('lc-set').value||'SET',
      variant:g('lc-variant').value||'VARIANT', cert:g('lc-cert').value||'0000-ALR',
      grade:g('lc-grade').value||'10', gl:g('lc-grade-label').value||'PRISTINE',
      body:g('lc-cbody').value, zone:g('lc-czone').value, acc:g('lc-caccent').value,
      gcol:g('lc-cgrade').value, text:g('lc-ctext').value, muted:g('lc-cmuted').value };
  }
  function dk(hex,a) {
  ...
  function front(v) {
    ...
    return '<svg width="70mm" height="20mm" viewBox="0 0 700 200" xmlns="http://www.w3.org/2000/svg">'
      +'<defs><clipPath id="m"><rect x="0" y="0" width="700" height="200" rx="4"/></clipPath>'
      ...
      +'<g clip-path="url(#m)"><g clip-path="url(#cl)">'
      ...
      +'<text x="10" y="61" font-size="28" font-weight="700" fill="'+v.text+'" font-family="Courier New,monospace">'+esc(v.name)+'</text>'
      +'<line x1="10" y1="67" x2="510" y2="67" stroke="'+dl+'" stroke-width="1"/>'
      +'<text x="10" y="88" font-size="19" font-weight="700" fill="'+v.muted+'" font-family="Courier New,monospace">'+esc(v.set)+'</text>'
      ...
      +'<text x="10" y="133" font-size="16" font-weight="700" fill="'+v.muted+'" letter-spacing="1.2" font-family="Courier New,monospace">CATEGORY</text>'
      +'<text x="10" y="150" font-size="19" font-weight="700" fill="'+v.text+'" font-family="Courier New,monospace">TRADING CARD</text>'
      +'<line x1="262" y1="123" x2="262" y2="154" stroke="'+dl+'" stroke-width="1"/>'
      +'<text x="267" y="133" font-size="16" font-weight="700" fill="'+v.muted+'" letter-spacing="1.2" font-family="Courier New,monospace">LANGUAGE</text>'
      +'<text x="267" y="150" font-size="19" font-weight="700" fill="'+v.text+'" font-family="Courier New,monospace">ENGLISH</text>'
      +'<line x1="10" y1="155" x2="510" y2="155" stroke="'+dl+'" stroke-width="1"/>'
      +'<text x="10" y="169" font-size="16" font-weight="700" fill="'+v.muted+'" letter-spacing="1.2" font-family="Courier New,monospace">CERT NUMBER</text>'
      +'<text x="10" y="187" font-size="19" font-weight="700" fill="'+v.text+'" font-family="Courier New,monospace">'+esc(v.cert)+'</text>'
      +'</g><g clip-path="url(#cr)">'
      ...
      +'<text x="611" y="29" text-anchor="middle" font-size="15" font-weight="700" fill="'+v.muted+'" letter-spacing="1.8" font-family="Courier New,monospace">ARCHIVE GRADE</text>'
      +'<line x1="528" y1="35" x2="688" y2="35" stroke="'+v.acc+'" stroke-width="0.7" opacity="0.35"/>'
      +'<text x="611" y="128" text-anchor="middle" font-size="110" font-weight="700" fill="'+v.gcol+'" font-family="Courier New,monospace">'+esc(v.grade)+'</text>'
      +'<line x1="528" y1="168" x2="688" y2="168" stroke="'+dl+'" stroke-width="1"/>'
      +'<text x="611" y="184" text-anchor="middle" font-size="17" font-weight="700" fill="'+v.muted+'" letter-spacing="3.5" font-family="Courier New,monospace">'+esc(v.gl)+'</text>'
      +'<line x1="528" y1="190" x2="688" y2="190" stroke="'+v.acc+'" stroke-width="0.7" opacity="0.35"/>'
      +'</g></g><rect x="1.5" y="1.5" width="697" height="197" rx="3.5" fill="none" stroke="url(#bg)" stroke-width="5"/></svg>';
  }
  function back(v) {
    var div=dk(v.acc,40);
    return '<svg width="70mm" height="20mm" viewBox="0 0 700 200" xmlns="http://www.w3.org/2000/svg">'
      ...
      +'<text x="350" y="92" text-anchor="middle" font-size="68" font-weight="700" fill="'+v.text+'" letter-spacing="6" font-family="Courier New,monospace">ALR</text>'
      +'<text x="350" y="116" text-anchor="middle" font-size="19" font-weight="700" fill="'+v.muted+'" letter-spacing="5" font-family="Courier New,monospace">INITIATIVE</text>'
      +'<line x1="274" y1="123" x2="426" y2="123" stroke="'+v.acc+'" stroke-width="0.6" opacity="0.4"/>'
      +'<text x="350" y="140" text-anchor="middle" font-size="14" font-weight="700" fill="'+dk(v.muted,20)+'" letter-spacing="2" font-family="Courier New,monospace">ARCHIVE \u00b7 GRADE \u00b7 CERTIFIED</text>'
      ...
      +'</g><rect x="1.5" y="1.5" width="697" height="197" rx="3.5" fill="none" stroke="url(#bg)" stroke-width="5"/></svg>';
  }
  function fold(v) {
  ...
  function lcRender() {
    var v=getV(), el=document.getElementById('lc-preview');
    if(!el) return;
    if(lcTab==='both') {
      el.innerHTML='<div class="lc-preview-label">Front \u2014 fold line \u2014 Back</div>'
        ...
        <div class="lc-body">
          <div class="lc-panel-left">
            ...
            <div class="lc-field-group">
              <div class="lc-field-label">Card Name <span class="lc-req">*</span></div>
            ...
            <div class="lc-two">
              <div class="lc-field-group">
                ...
                <input class="lc-input" type="number" id="lc-grade" value="10" oninput="lcUpdate()" />
              </div>
              <div class="lc-field-group">
